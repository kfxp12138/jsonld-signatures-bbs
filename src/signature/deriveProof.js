...
// import { getProofs, getTypeInfo } from "./util";

const jsonld = require("jsonld");
const { SECURITY_PROOF_URL } = require("jsonld-signatures");

const { getProofs, getTypeInfo } = require("./util");
...
const deriveProof = async (
  proofDocument,
  revealDocument,
  { suite, documentLoader, expansionMap, skipProofCompaction, nonce }
) => {
  if (!suite) {
  ...
  // jsonld.addValue(derivedProof.document, "revealIndicies",  derivedProof.revealIndicies);
  // console.log("derivedProof111", derivedProof);
  // console.log("derivedProof222", derivedProof.document);
  // console.log("derivedProof333", derivedProof.revealIndicies);
  return derivedProof.document;
};

module.exports = { deriveProof };